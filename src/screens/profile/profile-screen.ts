...
class ProfileScreen extends HTMLElement {
  ...
  connectedCallback() {
    ...
    const savedTab = shadow.querySelector("#saved-tab");
    const gamesTab = shadow.querySelector("#games-tab");
    const statsTab = shadow.querySelector("#stats-tab");
    const editProfileBtn = shadow.querySelector(".edit-profile-btn");
    const contentSections = shadow.querySelectorAll(".tab-content");
  ...
  renderSavedPosts() {
    ...
    if (!state.isAuthenticated) return `<p class="empty-message">Log in to save posts</p>`;
    const savedPosts = state.posts.filter(post => post.isSaved);
    if (savedPosts.length === 0) return `<p class="empty-message">No saved posts yet</p>`;
  ...
  renderGames() {
    ...
    if (games.length === 0) return `<p class="empty-message">No favorite games yet</p>`;
    return `
  ...
  render() {
      ...
      <div class="profile-container">
        ${!state.isAuthenticated 
          ...
          : `
            ...
            ${this.showEditModal ? `<edit-profile-modal></edit-profile-modal>` : ''}
          `
        }
      </div>
    `;
    ...
    const shadow = this.shadowRoot!;
    const loginBtn = shadow.querySelector("#login-btn");
    const editModal = shadow.querySelector("edit-profile-modal");

    loginBtn?.addEventListener("click", () => navigate(Screens.LOGIN));
